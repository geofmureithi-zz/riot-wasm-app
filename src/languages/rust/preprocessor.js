...
registerPreprocessor('javascript', 'rust', (code, options) => {
  ...
  const autogenMod = path.resolve('.cache/rust/src/autogenerated/mod.rs');
  function writeModuleToFile(module, code) {
    fs.mkdir(path.resolve('.cache/rust/src/autogenerated/'), { recursive: true }, (err) => {
        if (err) throw err;
        const stream = fs.createWriteStream(path.resolve(`.cache/rust/src/autogenerated/${module}.rs`));
  ...
  function appendModuleToLib(module) {
      fs.readFile(autogenMod, function(err, cont) {
          if (err)
              throw err;
              `${cont}`.indexOf(`mod ${module};`)>-1 ? null : fs.appendFileSync(autogenMod, `mod ${module};\n`);     
      });
  }
